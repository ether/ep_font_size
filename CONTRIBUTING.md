# Contributing to ep_font_size

Thanks for helping improve `ep_font_size` — a plugin for [Etherpad](https://etherpad.org/). LLM/Agent contributions are explicitly welcome, provided they follow the rules below.
...
`ep_font_size` is a plugin, so you develop it inside a checkout of [ether/etherpad](https://github.com/ether/etherpad).
...
# Install ep_font_size from your local clone
pnpm run plugins i --path ../ep_font_size

# Start the dev server (port 9001)
pnpm --filter ep_etherpad-lite run dev
```

Edits in `../ep_font_size/` are picked up after a server restart.
...
* Etherpad version, ep_font_size version, browser + OS
* Steps to reproduce
